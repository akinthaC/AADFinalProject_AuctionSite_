$(document).ready(function () {
    $.ajax({
        url: "http://localhost:8080/api/listings/Land/active", // Adjust URL if needed
        method: "GET",
        dataType: "json",
            ...
            if (Array.isArray(response.data)) {
                ...
                response.data.slice(0, 8).forEach(listing => {
                    ...
                    <div class="auction-card">
                        <img src="/static/uploads/${listing.mainImage}" alt="${listing.title}">
                        <h3>${listing.title}</h3>
                        <p>Starting at $${listing.startPrice}</p>`;

                        // Calculate auction end time
                        const endTime = new Date(new Date(listing.auctionStartDate).getTime() + listing.auctionDuration * 86400000);
    ...
    $.ajax({
        url: "http://localhost:8080/api/listings/Land/pending",
        method: "GET",
        dataType: "json",
        success: function (response) {
            ...
            if (Array.isArray(response.data)) {
                response.data.slice(0, 8).forEach(listing => {
                    ...
                    let auctionCard = `
                    <div class="auction-card">
                        <img src="/static/uploads/${listing.mainImage}" alt="${listing.landName}">
                        <h3>${listing.landName}</h3>
                        <p>Starting at $${listing.startPrice}</p>
                        ${timerText}
                    </div>`;
